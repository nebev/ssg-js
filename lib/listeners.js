...
exports.listen = function( screen, searchField, searchList, get_data) {
	...
	screen.key(['C-r'], function(ch, key) {
		...
		digitalocean.doctlComputeDropletList(true, function () {
			screen.leave();
			setTimeout(function () {
	...
	screen.key(['enter'], function(ch, key){
		...
		if (hostname[0].indexOf('A:') === 0) {	// TODO: This is a really shit way of doing this.
			...
			currentForm = do_list_form.getForm(hostname[0], function(result){
				screen.leave();
				var child = screen.spawn('/usr/bin/ssh',[result.username + '@' + hostname[3]]);
				child.on('close',function(){
					return process.exit(0);
				});
			});
			...
			screen.leave();
			var child = screen.spawn('/usr/bin/ssh',[hostname[1] + '@' + hostname[3]]);
			child.on('close',function(){
				return process.exit(0);
			});
		}
